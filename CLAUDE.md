...
## 2.3 コメント・返信の制限ルール（設計仕様）
LOGOSは「質の高い議論」を目的とするため、以下の制限を設計方針として定める。
- コメント: 1ユーザーにつき1トピックあたり **最大5件** まで（感情的なスパム・荒らし防止）。
- 返信（補足）: 同一コメントに対して **1ユーザー1回** のみ（議論の拡散防止）。
- エビデンス補足: 投稿者本人が **1回だけ** 補足を追加できる（`supplement` カラム）。既に実装済み。
- ※ コメント件数制限（max:5）はフロントエンドの制御のみで、バックエンドのバリデーションは未実装（将来対応）。
...
- 下書き保存時: OGPスクレイピング・通知をスキップし高速処理。ダッシュボードの「下書き」タブへリダイレクト。
...
- ダッシュボード: 「下書き」タブに件数バッジ付きで一覧表示。各下書きに「編集・本投稿」「削除」ボタン。
...
- [ ] UI/UXの総点検: マイクロインタラクション（アニメーション）の追加。下書きUIの細部調整（モーダルのフォームリセット等）。
- [x] エッジケースのテスト: バリデーション強化（文字数オーバー・不正URL等）とPHPUnit **31ケース・65assertions** 実装・全グリーン確認済み。
