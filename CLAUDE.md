...
## ローカル開発環境（WSL2 + Laravel Sail）の実行方法
- ローカルはLaravel Sail（Docker）で動いている
- SailコンテナのPHPは8.5だが、composer.jsonに
  platform: {"php": "8.3.30"} 設定済みのため
  composer.lockはPHP 8.3互換で生成される（さくら本番と互換）
- composerコマンドは必ずSail経由で実行する:
  ./vendor/bin/sail composer [コマンド]
- phpコマンドも同様:
  ./vendor/bin/sail php artisan [コマンド]
- Sailの起動: ./vendor/bin/sail up -d
- Sailの停止: ./vendor/bin/sail down
- フロントエンド: ./vendor/bin/sail npm run dev
- php単体コマンドはWSL上では使えない（command not found になる）

## フェーズ2 Step1完了（2026-03-19）
- Laravel JSON APIエンドポイント追加完了
- GET /api/topics（トピック一覧・ページネーション）
- GET /api/topics/{topic}（トピック詳細）
- GET /api/user/me（認証済みユーザー情報・auth:sanctum）
- laravel/sanctum ^4.3 インストール済み
- 本番（gs-f04.sakura.ne.jp）での動作確認済み

# 9. フェーズ2進捗（2026-03-20時点）
...
- Step2: Next.js新規作成完了
  ...
  - ローカルパス: ~/logos-next
  - Vercelデプロイはまだ（CORS設定が先に必要）

- Step3: 画面移行（一覧・詳細）完了
  - app/page.tsx: トピック一覧ページ（APIからデータ取得済み）
  - app/topics/[id]/page.tsx: トピック詳細ページ
  - 環境変数API_BASE_URL導入済み
  - .env.local: API_BASE_URL=http://localhost（ローカル用）
  - 本番用はVercel環境変数でAPI_BASE_URL=https://gs-f04.sakura.ne.jp を設定予定
...
- Step4: 認証実装完了
  ...
  - app/login/page.tsx（Laravel版忠実再現・eKYC/SNSボタンUI含む）
  - components/AppLogo.tsx（共通ロゴコンポーネント）
  - ヘッダー認証状態反映（通知ベル・アバタードロップダウン・カテゴリ管理）

## 次のチャットでやること（優先順）
1. 通知バッジの「0」非表示修正（未読0時は表示しない）
2. サイドバー実装（Laravel版sidebar.blade.phpを忠実移植）
3. Vercel環境変数にNEXT_PUBLIC_API_BASE_URLを追加

## Next.js開発の起動方法
cd ~/logos-next
npm run dev  # または ./node_modules/.bin/next dev
